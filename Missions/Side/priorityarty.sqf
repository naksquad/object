...
//-------------------- 1. FIND POSITION
	...
	_accepted = false;
	while {!_accepted} do {
		_position = [[[getMarkerPos currentAO,1500]],["water","out"]] call BIS_fnc_randomPos;
		...
		if ((_flatPos distance (getMarkerPos "respawn_west")) > 2000 && (_flatPos distance (getMarkerPos currentAO)) > 1200) then
		{
			_accepted = true;
		};
	};
