...
//-------------------- FIND SAFE POSITION FOR MISSION
	...
	while {!_accepted} do {
		...
		if ((_flatPos distance (getMarkerPos "respawn_west")) > 3500 ) then {
			_accepted = true;
		};
	};
...
//-------------------- BRIEFING
    ...
    [west,["hqCoastTask"],["OPFOR have been smuggling explosives onto the island and hiding them in a Mobile HQ on the coastline.We've marked the building on your map; head over there and secure the current shipment. Keep well back when you blow it, there's a lot of stuff in that building.","Side Mission: Secure Smuggled Explosives","sideCircle"],(getMarkerPos "sideCircle"),0,0,true,"search",true] call BIS_fnc_taskCreate;
