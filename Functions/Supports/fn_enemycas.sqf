...
_casArray = ["O_Plane_Fighter_02_Stealth_F","I_Plane_Fighter_03_AA_F","O_Plane_CAS_02_dynamicLoadout_F","I_Plane_Fighter_04_F","O_Plane_Fighter_02_F"];
...
if ((count enemyCasArray) < _jetLimit) then {
	...
	if (isNull enemyCasGroup) then {enemyCasGroup = createGroup resistance;};
	
	_jetPilot = enemyCasGroup createUnit ["I_pilot_F",[0,0,(1000 + (random 1000))],[],0,"NONE"];
	...
	[(units enemyCasGroup)] call AW_fnc_setSkill5;
	[enemyCasGroup,_aoPos] call BIS_fnc_taskAttack;
//	[enemyCasGroup, _aoPos, 5000] call BIS_fnc_taskPatrol;
